...
(() => {
  ...
  function readBodyValueSync(body, contentTypeHint = "") {
    try {
      ...
      if (body instanceof URLSearchParams) return clampText(body.toString());
      ...
      if (typeof body === "object") return "[non-string body]";
    } catch {
      ...
      try {
        ...
        try {
          ...
          if (!this.responseType || this.responseType === "text") {
            if (!isTextContentType(ct)) resText = ct ? `[binary ${ct}]` : "[binary body]";
            else resText = clampText(this.responseText);
          } else {
