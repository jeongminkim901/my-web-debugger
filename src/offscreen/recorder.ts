...
const DEFAULT_CLIP_PRE_MS = 10_000;
const DEFAULT_CLIP_POST_MS = 10_000;
...
let chunkBuffer: { ts: number; data: BlobPart }[] = [];
let activeMarks: {
  tabId: number;
  meta: { statusCode?: number; url?: string; at: number };
  pre: BlobPart[];
  future: BlobPart[];
  endAt: number;
  maxClips: number;
...
async function listClips(tabId: number) {
  const db = await openDb();
...
async function enforceClipLimit(tabId: number, maxClips: number) {
...
function addChunk(data: BlobPart) {
  const ts = Date.now();
  chunkBuffer.push({ ts, data });
  const cutoff = ts - DEFAULT_CLIP_PRE_MS;
  while (chunkBuffer.length && chunkBuffer[0].ts < cutoff) chunkBuffer.shift();
  if (activeMarks.length) {
    for (const mark of activeMarks) {
      if (ts <= mark.endAt) mark.future.push(data);
    }
    finalizeExpiredMarks(ts);
  }
}

function capturePreChunks(now: number, preMs: number) {
  const cutoff = now - preMs;
  return chunkBuffer.filter((x) => x.ts >= cutoff).map((x) => x.data);
}

function markClip(tabId: number, meta: { statusCode?: number; url?: string; at: number }, preMs: number, postMs: number, maxClips: number) {
  ...
  if (activeMarks.length > 6) activeMarks = activeMarks.slice(-6);
}

function finalizeExpiredMarks(now: number, force = false) {
...
async function startRecordingWithStreamId(tabId: number, streamId: string) {
