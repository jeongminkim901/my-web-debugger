...
(() => {
  ...
  const envBox = el<HTMLElement>("envBox");
  ...
  let timelineNetMap = new Map(); // id -> item
  ...
  if (timelineEl) {
    timelineEl.addEventListener("click", (e) => {
      const target = e.target as HTMLElement | null;
      const bar = target?.closest?.(".timeline-bar") as HTMLElement | null;
      if (!bar) return;
  ...
  if (screenshotWrap) {
    screenshotWrap.addEventListener("click", (e) => {
      ...
      if (Number.isFinite(idx)) return openShotModalAt(idx);
  ...
  function sameOrigin(url) {
    if (!hiliteOrigin) return true; // origin 모르면 제한 안 함
  ...
  function isNetworkInWindow(item, centerTs, windowMs) {
    ...
    if (typeof s === "number") return Math.abs(s - centerTs) <= windowMs;
    if (typeof e === "number") return Math.abs(e - centerTs) <= windowMs;
    return false;
  }
    ...
    if (envBox) {
      if (!env) {
        ...
        const osLabel = (() => {
          ...
          if (!isWin) return env.platform || env.uaData?.platform || "-";
          let version = "";
  ...
  function makeConsoleKey(x) {
    ...
    const lvl = x.level ?? "";
    let msg = "";
    try { msg = JSON.stringify(x.args ?? []); } catch { msg = String(x.args ?? ""); }
  ...
  function sanitizeForRaw(data: any) {
